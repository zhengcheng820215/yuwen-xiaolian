...
状态：`DESIGN ACCEPTED / ENGINEERING PARTIALLY IMPLEMENTED`

文档版本：`real_learning_data_collection_and_observation_v1.2`
...
以下内容由本文冻结设计，但不得因文档存在而标记为工程完成：

- 多使用者匿名档案与选择入口；
- 题目展示、首次输入、有效停留、返回修改和主动放弃事件；
- 成人帮助与家长简短观察；
- Learning 提交自动转换为匿名题目校准样本；
- 家长学习记录页、题目观察页和原始审计页的正式分层；
- 数据导出、导入恢复、按使用者删除与保留期限控制；
- 跨浏览器、跨设备或服务端集中同步。

### 2.3 最小采集链

第一阶段继续使用现有固定产品身份 `student-local-primary-v1`，只接入五个核心事件：
...
6. **本地优先与可控。** 当前封闭 Beta 默认本机保存；可查看、导出和删除，不静默上传。
...
P4、P5 各自增加独立验收，不得以家长报告或多用户尚未完成阻塞 P1、P2 的真实数据采集。
...
正式 Learning 已完成五事件最小链、稳定提交身份、Outbox 恢复、匿名校准接续和内部完整性核对，`WP0—WP7` 工程与浏览器验收通过，可以进入固定单学生的受控真实运行。该结论只代表采集工程就绪，不代表已经获得足量真实样本或完成群体校准。过程时间、帮助程度、家长展示与多使用者继续保留正式边界，并按 P4、P5 后置实现。

P1 与 P2 是34道题开始形成可信真实数据的当前阻塞项；P3用于确认采集没有静默缺失。家长报告和多使用者分别延后到 P4、P5，不阻塞第一批真实 Learning。

在 P1、P2 完成前，现有数据仍只能理解为“学习闭环记录”；完成后可以形成版本化真实 Attempt，但在样本不足时仍不能宣称题目已经得到稳定实证校准。
