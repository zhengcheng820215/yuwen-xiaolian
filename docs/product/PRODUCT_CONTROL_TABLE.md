...
当前已提交源码身份统一以 `git rev-parse HEAD` 复读，不再在本控制表复制易漂移的固定提交号。最近一次历史完整冻结验收基线为 `5a6892e30d29634d007f2102aef481d6cd61f156`；当前本地基线已完成 Revision 1964 收口、DeepSeek 真实 Provider Smoke、Preflight / Launch Record / Trial Identity Binding 重新准入与显式激活。Trial 当前限定为单学生和 8 项 `observe_only` Adapter；不得把这一运行状态上推为当前 Product Acceptance 或 Live 结论，即时状态继续唯一服从[当前产品状态](./CURRENT_PRODUCT_STATE.md)。
...
2. WP7B 已 Engineering PASS，当前 Preflight / Launch / Binding 已准备并显式激活；保持单学生与 `observe_only` 范围
3. 只组织至少 5 次受控真实学生验收，不扩建 `/practice/knowledge` 第二产品壳
