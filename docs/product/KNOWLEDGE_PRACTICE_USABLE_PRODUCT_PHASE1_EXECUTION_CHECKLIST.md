...
- [x] 输出WP7B-5真实学生受控产品验收准备与执行包。
- [x] 冻结工程基线提交、候选Build Identity、批次ID和变更作废规则。
- [x] 冻结T1—T5场景、设备矩阵、统一任务指令、观察表和停止规则。
- [x] 复核539/539自动化、Production Build和 `/learning` 入口状态。
- [ ] 从冻结提交建立干净试用工作区并记录实际Build Identity。
- [ ] 完成 P4-A01—A06 对应的 Commit、Runtime、Provider、Formal Store、Session恢复、Launch Record 与 Trial Binding 前检。
- [ ] 使用物理键盘完成Enter/Space补验，并完成浏览器真实200% Zoom补验。
- [ ] 产品负责人确定至少 5 次试用的学生与时间。
- [ ] 每次试用使用统一任务脚本。
- [ ] T1先完成无指向入口观察，分别记录默认入口理解与知识练习可发现性。
- [ ] 记录学生是否能独立开始并完成。
- [ ] 记录学生能否解释至少一条具体反馈。
- [ ] 记录学生是否误解本轮结论为长期能力判断。
- [ ] 记录中断恢复是否造成困惑。
...
- [x] 建立 Product Acceptance Report 证据容器；最终结论待5次有效试用后填写。
- [x] 冻结观察记录的受限存放、访问、脱敏、30天最长保留与删除规则。
