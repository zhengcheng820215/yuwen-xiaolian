...
WP7B-5 真实学生受控试用是 `knowledge_practice_single_entry` 及 Batch E01—E02 的限定验收子批次。它必须先完成 Batch A 身份与入口预检，但其 5 次试用结果不得替代 Batch B、C、D，也不得直接升级其他 `claimId` 或宣称 P4 总体完成。

WP7B-5 的无指向入口观察用于验证“核心阅读仍是默认主要学习，知识练习承担补充角色”；其后明确要求进入知识练习的步骤只验证知识练习自身的可发现性与会话完整性。两类证据必须分开记录。
