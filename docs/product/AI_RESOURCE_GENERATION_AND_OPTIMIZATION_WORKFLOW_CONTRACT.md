...
页面状态、主按钮、统计和刷新恢复必须消费同一个可见状态 Resolver。内部详细状态继续用于审计和恢复，不得逐项重新暴露成用户步骤。顶部固定使用互斥的“需处理 / 待采用 / 待发布 / 已发布”四类统计：缺少完整题目、Candidate 失效或任一阶段失败归入“需处理”；存在完整 Candidate 但尚未 Adopt 归入“待采用”；已经 Adopt 且检查、确认或发布尚未完成归入“待发布”；只有 Formal Resource 与 Registry 完整提交后才归入“已发布”。四项之和必须始终等于当前训练任务总数。
...
顶部“需处理、待采用、待发布、已发布”不得重新拼装一套生命周期。任务卡状态、主操作和顶部汇总必须消费同一份 `TaskProductionView` 与质量处理投影，并按互斥桶计数。

状态优先级固定为：

1. `published` 是终态事实，必须计入“已发布”；历史 Candidate 失败、旧操作结果或残留质量提示不得将已发布任务降级为“需处理”或“待发布”；
2. 非已发布任务存在当前质量提醒、Candidate 失效、采用失败或发布失败时，计入“需处理”；
3. 题目内容完整且已有可采用 Candidate、尚未创建正式 Revision 时，计入“待采用”；
4. 已进入 Revision 链且仍处于检查、确认或发布编排过程时，计入“待发布”；
5. 四项计数必须与当前任务卡逐项对应，且总和严格等于训练任务数量。

汇总回归必须覆盖“已发布任务仍残留旧 Candidate 错误 + 另一任务存在质量提醒 + 一项题目待采用”的组合，预期结果必须为“需处理 1、待采用 1、待发布 0、已发布 1”，不得发生桶位互换。
...
25. 未采用题目计入“待采用”；只有已经进入正式 Revision 链且尚未发布的题目才计入“待发布”；缺失、失效或失败任务计入“需处理”。
26. 顶部“需处理 + 待采用 + 待发布 + 已发布”必须等于当前训练任务总数，四类任务不得重复计数。
27. 已发布任务的终态优先于 Candidate 面板残留反馈；顶部汇总不得把卡片已显示“已发布”的任务计入其他桶。
28. 质量提醒待处理必须与任务卡一致计入“需处理”，不得仅因底层处于 `pending_confirmation` 而误计为“待发布”。
