...
2026-08-06 增补确认（2026-08-09 文案校准）：当 AI 训练任务规划结果已经包含符合 `QuestionEditableContent` 契约的完整题目内容时，系统必须基于该内容形成不可变的初始 Candidate，不得要求用户再次执行一次 AI 生成。页面此时显示“题目待采用”，主操作统一为“采用并发布”；用户对当前方案不满意时，可切换其他候选、放弃本轮方案，或按当前上下文重新生成题目。只有训练任务确实没有完整题目内容时，才显示“生成题目”。该调整不改变“采用后才创建 Revision”的底层边界。

2026-08-06 单人模式收敛确认：用户点击采用主操作即对当前可见 Candidate 作出明确采用决定。系统在创建 Question Revision 后，应自动串联 Validation、Assessment、Human Review Decision、Freeze、Formal Resource 与 Registry；正常无提醒路径直接完成发布。存在质量提醒、检查失败、确认失败或发布失败时必须中断为“需要处理”，不得自动接受提醒、伪造理由或回滚已经成功的领域阶段。多人审核模式仍可恢复为分阶段操作，但不改变底层命令和审计边界。
...
单次生成最多向用户展示三个 Candidate。页面只允许通过“方案1 / 方案2 / 方案3”切换查看一个当前方案，不提供多选、并排比较或候选合并；“采用并发布”只作用于当前选中的 Candidate。候选区同时提供中性的“放弃本轮方案”次操作，用于废弃本轮同批候选，不得修改原题、Draft Revision 或已发布正式资源。AI 优化、重新生成和异常纠错不得与采用动作并列为常驻入口，仅在无可用方案、方案过期或明确失败恢复时按上下文显示必要的生成或重试动作。
...
| 存在可用 Candidate，尚未采用 | 题目待采用 | 采用并发布 | 切换查看其他方案、放弃本轮方案 |
