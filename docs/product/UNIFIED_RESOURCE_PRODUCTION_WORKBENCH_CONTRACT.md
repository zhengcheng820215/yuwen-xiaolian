...
文档版本：`unified_resource_production_workbench_v1.8`
更新日期：2026-08-09
...
8. 候选生成、放弃和采用到编辑缓冲区继续遵守既有版本边界；只有任务组发生真实变化时才显示`保存任务组修改`，用于创建或更新任务组工作草稿并按规划契约冻结 Observation Plan Revision；单任务`采用题目`独立消费 QuestionCandidate 并进入 Draft Revision、Assessment、Human Review 与 Publication 链路；
