...
候选区最多提供三个方案切换入口、一个“放弃本轮方案”次操作和一个“采用并发布”主操作。不得提供多选采用、字段级人工编辑或对同一 Candidate 的重复确认。
...
3. Candidate 区最多显示三个互斥方案切换、一个“放弃本轮方案”次操作和一个“采用并发布”主操作；切换方案不产生 Revision，放弃不修改现有题目或正式资源；
...
| 展开决策区 | 质量提醒、候选切换、放弃本轮、采用并发布、失败恢复 | 第二套采用、确认、审核或发布入口 |
