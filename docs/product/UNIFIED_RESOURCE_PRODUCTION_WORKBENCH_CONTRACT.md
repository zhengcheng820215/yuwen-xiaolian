...
任务组级 TrainingTaskCandidate 使用“采用并保存”收口前台操作。系统依次执行候选采用计算、Observation Plan 工作草稿保存与结构检查；只有保存成功才关闭候选区。保存失败时候选保持可见，当前已保存任务组不变，并显示可重试反馈。“采用并保存”只形成训练任务计划，不创建或发布正式题目；QuestionCandidate 的“采用并发布”仍是后续单任务动作。
...
1. 当前单人模式页面按钮只表达用户意图，例如`保存任务组修改`和`采用并发布`；没有任务组变化时不得常驻保存按钮；
...
补充候选是独立临时容器，尚未加入当前任务组，不参与任务编号、任务总数、待发布/已发布统计或状态汇总。采用并保存成功后候选进入任务列表；放弃候选只关闭该容器，不修改当前已保存或已发布任务。
...
8. 候选生成与放弃继续遵守既有版本边界；候选采用使用`采用并保存`顺序调用采用命令与 Observation Plan 保存命令，成功后进入已保存任务组，失败则保留候选；后续人工增删或编辑产生真实变化时才显示`保存任务组修改`；单任务`采用题目`独立消费 QuestionCandidate 并进入 Draft Revision、Assessment、Human Review 与 Publication 链路；
...
4. “只显示有待处理的材料”仅在待处理数大于零时出现并过滤材料选择列表。当前材料即使已经完成也继续保留在列表中，避免筛选导致上下文消失；
5. 筛选和统计不得创建 Candidate、Revision、Assessment、Formal Resource 或 Registry Entry，也不得绕过未保存修改保护。
...
任务组操作文案统一为“生成整组替代方案”和“生成补充候选”。按钮前必须常驻说明：生成只创建候选，不立即修改当前任务；已审核或已发布任务受保护；替代方案采用后只替换其余任务，补充候选采用并保存后才追加到编辑任务组。候选预览仍负责展示具体覆盖差异和受保护数量，不增加生成前确认弹窗。

验收至少覆盖：发布中间任务后定位下一项、发布最后一项后回到前面未完成项、全部完成提示、筛选后保留当前材料、跨材料统计刷新一致、存在未保存修改时筛选不触发切换，以及整组生成前后当前任务组保持不变。
