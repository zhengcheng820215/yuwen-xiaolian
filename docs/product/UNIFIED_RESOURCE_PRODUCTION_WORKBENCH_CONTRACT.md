...
任务卡主状态只保留三类：`待处理`、`已确认`、`已发布`。真实处理原因作为卡内说明、Loading 或恢复提示存在，不得升级为新的主状态或顶部流程步骤：

| 卡片情形 | 卡片主状态 | 卡内原因 | 唯一主要动作 |
| --- | --- | --- | --- |
| 尚无完整题目 | 待处理 | 未生成题目 | 生成题目 |
| 已有当前方案或一个至三个 AI 新方案 | 待处理 | 题目可供判断 | 采用并发布 |
| Candidate 已采用，自动编排正在运行 | 已确认 | 正在检查或发布 | 无重复提交入口 |
| 检查或质量提醒需要人工判断 | 待处理 | 显示具体提醒 | 与当前提醒匹配的处理动作 |
| Candidate 已采用但正式写入未完成 | 已确认 | 发布未完成 | 重试发布 |
...
1. 统一 Resolver 新增 `pending / confirmed / published` 三态投影，对应用户文案“待处理 / 已确认 / 已发布”；
2. Candidate 待选择、题目缺失、结构问题和质量提醒统一归入“待处理”，具体原因仍在任务卡内显示；
3. Candidate 已采用后的检查、发布中断和可重试发布统一归入“已确认”，避免恢复“待审核、待最终确认、已确认待发布”等中间业务状态；
...
4. “保留当前题目”如仍作为质量提醒处理路径存在，其完整业务语义必须是“保留当前题目并发布”，且只能在没有并行 Candidate 时出现；Candidate 已存在后必须退出可见操作面；
...
状态文案继续遵循“顶部两态、卡片三态、原因就近说明”：顶部只显示“待发布 / 已发布”，卡片只显示“待处理 / 已确认 / 已发布”。“题目待选择”“有质量提醒”“正在发布”“发布未完成”等仅作为原因文案。面向普通用户的质量说明使用“评分标准”，不得直接显示 `Rubric` 等实现术语。
