...
题目工作台必须通过唯一展示状态解析页面名称，不允许组件各自拼接标题：

| 当前状态 | 页面名称 |
| --- | --- |
| 普通录入 | 题目录入工作台 |
| 草稿、检查失败、退回修改 | 题目修改与提交平台 |
| 待人工审核 | 题目人工审核平台 |
| 审核通过、待发布 | 题目发布平台 |
| 发布部分完成 | 题目发布恢复平台 |
| 已发布 | 已发布题目 |
...
页面副标题必须解释当前阶段的唯一主任务，不显示 Phase 编号或内部实现名称。
...
1. 页面名称覆盖录入、修改、待审、待发布、发布恢复、已发布和归档状态；
2. 页面名称与副标题只由同一状态解析函数生成；
...
1. “待人工审核”只统计 `pending_review`；
2. `drafted`、`validation_failed`、`revision_required` 不得计入待人工审核；
3. “审核通过”只统计 `reviewed`；
4. “已发布”只统计已存在有效正式资源关联的题目；
5. 包含多种状态的右侧导航统一命名为“本批题目”，不得称为“本批待审核题目”。
...
1. 只消费四项核心生命周期统计，不复制批次处理概览；
2. 使用不透明页面背景、明确分隔线和稳定层级，滚动时不得与正文叠加；
3. 状态统计、题目列表标签和页面标题继续消费同一份展示状态投影；
4. 桌面端和窄屏下均不得因数字、标签或动态状态变化改变容器高度；
5. 自动化或浏览器验收必须确认“批次处理概览”不位于任何 `sticky` 容器内。

### 20.7 审核与发布步骤固定外显

题目完成录入检查、进入后续流程后，页面必须固定外显以下三个阶段：
...
正式发布完成后，页面不再存在“当前进行中步骤”。步骤 2、3 保持灰色，步骤 4 改为绿色
`已完成发布`，用于表达最终成功结果；此时不得继续使用蓝色当前态，也不得再次显示发布按钮。
...
| `pending_review` | 3 |
| `reviewed` 或发布准备处理中 | 4 |
| `published` | 无当前步骤；步骤 4 显示绿色 `已完成发布` |

验收至少覆盖从步骤 2 提交后切换到步骤 3、审核通过后切换到步骤 4，以及刷新页面后仍能根据
当前 Draft 与发布状态恢复同一高亮步骤。发布成功后还必须验证步骤 4 切换为绿色完成态，
页面不存在蓝色当前步骤和重复发布入口。
