...
反馈对象必须与 Frozen Task 的真实观察对象一致。Feedback Adapter 应优先使用题目元数据、训练动作、题干限定对象和 Rubric 共同确定学生端语义，至少区分人物心理、人物特点、景物或事物状态、词句表达效果、句段结构关系、事情原因与主要内容。题干出现“特点”不得自动归入“人物特点”；只有题目明确以人物、人物行为或“怎样的人”为观察对象时，才允许生成“人物特点”类文案。诸如“春天刚睡醒的特点”“景物共同表现的状态”必须投射为景物、事物或整体状态，不得出现“人物特点”。旧题缺少结构化对象时可以使用确定性兼容推断，但低置信度结果必须退回克制表述，不得猜测对象类别。
...
关键词命中只能作为候选线索，不能单独证明某项 Requirement 已完成。结论覆盖至少还需要正式 Diagnosis / Positive Evidence 对该方向的确认；文本依据必须能在学生答案中定位到完整事实片段；关系说明必须由学生答案中的解释关系或与之对齐的正式结果共同支持。某个环节已经覆盖时可以在“已经做到”中单独确认，但不得把“结论已写出”扩大解释为整道题已经完成。只有所有必要 Requirement 均为 `covered`，页面才可以只展示完成点评而不展示主要缺口。
...
Rubric 反馈还必须保持观察目标、局部证据、关系动作与内部结论方向的角色稳定。`acceptedKeywords` 只表示可接受信号，不能因为学生命中一个材料词语，就把该词自动提升为最终观察结论；局部证据之间也不得被错误投射为互相解释的目标。“先补这一点”只描述主要断点，“下一步这样做”只提供一个不泄露答案的具体思考动作，两者不得同义复述。详细工程边界见 [RUBRIC_FEEDBACK_SEMANTIC_ROLE_ALIGNMENT_ENGINEERING_AND_DEBUG_ACCEPTANCE.md](./RUBRIC_FEEDBACK_SEMANTIC_ROLE_ALIGNMENT_ENGINEERING_AND_DEBUG_ACCEPTANCE.md)。
...
- 禁止重复提交；
- 显示正在提交或分析；
- 对一次完整的浏览器到 Diagnosis Application Boundary 请求设置有限等待，不允许页面无限停留在“正在提交并分析”；
...
Application Boundary 的学生可见等待上限必须覆盖当前 Provider 的正常重试预算，并在达到上限后中止浏览器等待。超时不等于回答无效，也不形成 Diagnosis、Evidence 或能力结论；页面必须退出忙碌态、持久化当前草稿，并进入可恢复的“重新分析”状态。重新分析必须复用同一 `operationId / responseId / diagnosisRequestId`，不得创建第二次正式作答。浏览器已经判定超时、学生已返回修改或已开始新的重试后，原请求的迟到响应不得覆盖当前页面状态；服务端后续若已形成同一 requestId 的正式提交，重试应按既有幂等规则复用该结果。

Provider 单次尝试超时属于 Runtime 内部策略，不直接投射为学生界面的倒计时或工程错误。学生端只表达“本次分析等待时间较长，回答已保留”，并提供“重新分析 / 返回修改”；不得要求刷新页面，不得让提交按钮永久禁用。

无效作答、分析重试与下一任务资源缺口是三个互斥状态，正式页面必须按下表投影，不得仅因它们都具有“暂时不能继续”的表象而复用按钮或文案：

| 状态 | 成立条件 | 是否调用 Diagnosis | 页面主动作 | 禁止行为 |
| --- | --- | --- | --- | --- |
| 补充回答 | 空白、占位、复制、随机拼接或与当前任务无关，尚未形成最低有效回应 | 否；若确定性 Gate 无法识别而由语义 Gate 识别，也不得形成正式结论 | `修改并重新提交` | 不得显示“重新分析”，不得匹配下一任务 |
| 重新分析 | 回答已经通过有效性 Gate，Diagnosis Provider 或 Application Boundary 发生可重试临时故障 | 首次调用失败；重试复用同一 Operation 和回答 | `重新分析`，次操作 `返回修改` | 不得伪装为资源缺口，不得创建新的 Response 或 Operation |
| 下一任务缺口 | 本轮 Diagnosis、Evidence 和持久化均已正式完成，仅下一任务匹配无结果 | 本轮已经且只调用一次 | `检查下一任务`或`返回学习入口` | 不得重新调用 Diagnosis，不得显示“返回修改 / 重新分析” |

达到最低字数不等于有效作答。对长篇随机中文、输入法联想词拼接、复制无关段落等“形式上有字数、语义上没有回应任务”的输入，系统必须优先使用确定性高置信规则拦截；确定性规则不确定时，语义有效性结论仍必须在 Evidence 和下一任务决策之前形成安全 Gate。
...
补充回答状态必须保留可编辑原文，并明确使用“修改并重新提交”语义。它不属于已提交答案的处理恢复，因此不得锁定输入框，不得显示“重新分析”；也不允许以“当前没有对应任务”结束本轮。
...
5. 反馈、总结和过渡信息不得重复；
6. Learning 学生端的品牌强调、当前状态、学习推进动作与操作焦点统一使用绿色；
7. 绿色既可以表达学生侧开始、继续、保存和提交，也可以表达正式成功；具体含义必须由文案和组件形态共同说明；
8. 红色只表示明确阻断或错误；
9. 不使用游戏化、装饰插图或多层卡片填充页面；
10. 学生无需理解工程术语即可继续。

上述绿色规则只适用于 Learning 学生平台。资源录入、审核与发布工作台继续遵循独立的蓝色常规动作、紫色 AI、绿色结果状态规范。
...
- 新学习会话不得继承已结束会话的材料语境；具体 Frozen Version 历史用于新题优先，资源池耗尽后允许轮换复习，但不得重复当前活动 Session 已使用的任务；
- 到期复测没有合格正式资源时应降级进入普通训练，不得让不可执行的复测计划锁死“开始新的学习”；
...
- 未用版本和安全复习版本均不可匹配时，资源耗尽继续显示正常空状态，不能把 `already_used` 伪装成读取异常；
...
录入端已经发布的普通 Training 题组进入 `/learning` 后，必须在首次正式任务确定时冻结为当前活动会话的稳定任务队列。队列按正式顺序规划消费同一 Material 下最多 `5` 道当前可执行版本；基础理解单选、局部解释和综合分析可以按规划形成梯度，但每道题仍保持独立 Response、Diagnosis、Evidence 与反馈。

当前队列尚有未消费任务时，下一题解析优先使用队列中的明确 `resourceVersionId`，不得重新用能力默认值寻找另一份资源，也不得因自适应 Strategy 暂时没有能力完全匹配项而提前返回入口。完成队列最后一题后，系统才结束本次会话；Retest / Transfer 和跨材料调度进入下一 Session，不插入并改写当前题组身份。

若队列中的正式版本在会话期间失效，系统应保留已完成结果并明确提示“下一题版本已不可用”，不得静默跳过、换题或只显示“返回学习入口”。旧活动上下文没有队列时，可以在不修改既有 Round 的前提下，根据当前 Round 所绑定的 Frozen Version 一次性补建队列，用于安全恢复。
...
### 18.2 开始命令返回态与工作区进入门禁（2026-08-12）

1. `/learning` 点击主要操作后必须保存并消费 `startOrResumeUnifiedLearning()` 返回的最新 `UnifiedLearningEntryState`；
2. 只有最新状态 `canEnterWorkspace = true` 时才能进入正式工作区；不得依据点击前缓存状态直接切换视图；
3. 若开始期间正式资源、Registry、Session 或复测优先级变化，页面必须停留入口并展示最新状态及其唯一操作；
4. 不允许出现入口“可以开始”、工作区“无任务”、返回入口后再次“可以开始”的循环；入口探测、开始命令和工作区任务准备必须使用同一可用性语义；
5. 开始按钮在命令进行期间保持单次提交，错误或状态变化后恢复为最新状态对应动作；
6. 浏览器验收必须覆盖新发布 Formal Resource 可被实际匹配、开始后形成 Concrete Learning Task，以及开始前后资源变化时不会进入空工作区。
