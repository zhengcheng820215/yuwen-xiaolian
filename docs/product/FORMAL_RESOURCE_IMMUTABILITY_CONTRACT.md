...
当已发布正式版本与新版 Candidate 同时展示时，正式版本只能标记为“当前正式版本”并作为只读比较基线。它不是 `QuestionCandidate`，不得被复制、降格或包装成可采用 Candidate，也不得响应“采用并发布”。用户继续保留当前正式版本时无需执行任何命令；只有选中新版 Candidate 并采用后，系统才创建后继 Revision 并尝试发布新正式版本。在 V2 完整发布并切换 Active Registry 前，V1 始终保持当前有效。

页面不得为已发布资源恢复直接编辑、保存、重新确认或覆盖发布入口。唯一允许的变化路径是“生成新版方案 -> 选择新版 Candidate -> 采用并发布新版本”。新版发布失败时旧 Active Registry 继续指向原正式版本，页面仅提供恢复当前新版发布的动作，不要求用户重新选择 Candidate，也不得修改旧正式版本。
...
11. 当前正式版本只参与只读比较，不进入 Candidate Repository，也不能触发重复采用或重新发布；
12. 用户未选择新版 Candidate 时不产生 Revision、Freeze、Formal Version 或 Registry 写入。
