...
后续校准发现，v5 题干直接写出“老百姓也开始说出真相”，把本应由学生观察的反应变化提前作为已知条件给出。该题应继续沿用同一 Question Lineage 生成 v6 后继版本，改为：

> 小孩子说出“可是他什么衣服也没有穿呀”后，人们的反应发生了怎样的变化？这对揭穿骗局有什么作用？

本次不是增加第二个独立任务，而是把观察顺序恢复为“识别人们反应的前后变化 → 解释这一变化怎样推动骗局暴露”。评分只覆盖这条短因果链：

- 变化识别：人们由不敢说真话、随声附和，转为私下传播并公开说出真相；
- 作用解释：集体沉默和附和被打破，真相扩散，骗局因此暴露。

只完成其中一步时应保留部分完成诊断；不得继续追加社会主题、皇帝性格或骗子手段等独立要求。

禁止：

- 把题目改成只复述孩子说了什么；
- 在题干中直接替学生说出“老百姓开始说真相”这一变化；
- 再要求独立概括社会主题；
...
### 7.1 v6 窄范围校准治理（2026-08-25）

- 治理范围仅限《皇帝的新装》`question-observation-task-plan-12ktvxo:v5`；
- 采用“先观察变化、再解释作用”的题干结构，保持原 Material、Training Task、主要 Ability 和 Question Lineage；
- 通过 successor Candidate 形成 v6，v5 仅转为 `superseded`，不原位覆盖 Frozen Resource；
- 正式存储 revision：`1962 -> 1963`；Registry Current Head 已切换为 `question-observation-task-plan-12ktvxo:v6`；
- 活动正式题、Learning 可消费题和 Frozen Quality Trace 均保持 `81`，新版本负担审计结果为 `focused_short / retain`；
- Learning 提示已按同一规范投射为“对照关键句前后人们的反应”，不再误用词句表达效果模板，也不提前泄露具体变化与结论；
- 重复治理返回 `apply-noop`；真实学生校准仍待 Trial 数据验证。
