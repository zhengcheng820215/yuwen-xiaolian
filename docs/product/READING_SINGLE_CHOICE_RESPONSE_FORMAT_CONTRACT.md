...
文档版本：`reading_single_choice_response_format_v1.10`
...
- 不实现部分得分、漏选权重或组合评分；
- 不实现选择后自动追问；
- 不要求所有材料无条件固定包含选择题；但用户明确通过补充生成建立基础理解入口、当前题组尚无单选且材料存在合格新观察点时，本轮应至少生成一道单选候选；
- 不按题型配额机械平衡题组；
- 不因选择正确直接宣称能力已经掌握；
...
系统不得规定选择题固定占比，也不得为了题型丰富度把高阶文本任务机械改成单选。生成器只能在训练动作适合时提出单选 Candidate；材料不支持有价值干扰项时，应继续使用文本格式。

为避免题组长期只有高输入负荷的阅读理解，生产工作台允许使用以下受限入口目标：当用户执行“生成补充候选”、当前题组没有 `single_choice` 且本轮仍有任务容量时，系统请求至少一个适合信息定位、对象关系、局部含义、简单因果或初步辨认的新单选观察。该目标不是放宽质量或重复门禁：

- 若候选只是在已有题干上更换 `responseFormat`，必须判为既有 Observation 的替代方案，不得作为补充任务采用；
- 若已有训练动作适合改成单选，应从该任务进入单题替代优化，采用后形成同一题目身份的后继 Revision；
- 若没有新的合格观察点，必须说明无法生成，不得返回一组纯文本候选并误报为已满足本次补充目标；
- 任务顺序仍由 Observation Plan 和 TrainingTask Role 决定，不固定把单选排在第一位。
