...
当同一材料或同一能力路径同时提供单项选择和文本作答时，Diagnosis 应利用两类任务的观察差异定位失败步骤，但不得把题型本身当作能力结论：

- 单项选择主要提供信息定位、关系辨认、局部理解和典型偏差信号；
- 短文本、长文本主要提供解释、概括、证据组织、推理、分析和表达信号；
- 两类任务必须各自产生独立 Response、Attempt、Diagnosis 和 Evidence；
- 跨任务判断只能引用这些独立证据，不得先合并成总分再反推根因。

互补解释遵循：

| 单选表现 | 文本表现 | Diagnosis 处理 |
| --- | --- | --- |
| 弱 | 弱 | 将信息定位或基础理解列为优先前置能力假设 |
| 强 | 弱 | 保留基础理解的正向信号，重点检查概括、证据组织、分析、推理或表达 |
| 弱 | 强 | 标记为 evidence conflict，检查题目质量、误触、猜测、格式适配和观察目标错位 |
| 强 | 强 | 形成多来源正向证据，但不跳过独立复测与迁移验证 |

以上结果默认只允许形成 `rootCauseHypothesis`。只有后续独立任务、修正记录、Retest 或 Transfer 提供持续一致证据时，Evaluation Runtime 才能提高根因或能力判断的置信度。

禁止做法：

- 因单选正确就把文本回答缺口归为“只是表达不好”；
- 因单选错误就忽略文本回答中已经展示的理解与推理；
- 用一个错误干扰项直接确认长期能力薄弱；
- 将不同 Material Version、不同观察对象或不可比较任务强行组成互补证据；
- 用题型正确率直接替代能力路径诊断。
