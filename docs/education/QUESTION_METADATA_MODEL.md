...
`answerAcceptance` 适用于能够建立明确答案接受边界的题目。

通常具备以下一个或多个特征：

- 存在明确的标准答案、候选答案或可接受答案集合；
- 学生答案可通过格式规范化、关键词覆盖或语义等价进行判断；
- 不需要完整分析开放式推理过程，即可判断答案是否基本成立；
- 主要评价目标是答案是否可接受，而不是推理过程、表达质量或深层能力表现；
- 需要区分严格匹配、候选答案匹配、关键词匹配和语义等价匹配。
...
这些只是典型场景，不代表 `answerAcceptance` 只能用于这些题型。
...
`answerAcceptance` 不负责评价：

- 需要多步推理才能成立的开放性答案；
- 主要考察论证结构、概括质量、鉴赏深度或表达质量的答案；
- 无法仅通过答案内容判断，需要结合完整作答过程的任务；
- 需要生成详细错因、能力诊断或成长证据的复杂作答。

典型场景包括概括、分析、推理和开放表达等，但具体是否适用取决于该题的 `answerAcceptance` 是否足以支撑判断。
...
- 答案需要覆盖若干核心信息点；
- 可以通过 required rubric items 判断是否抓住主要内容；
- 允许不同表达方式，但核心要点相对稳定；
- 评价重点是信息筛选、合并和组织，而不是唯一标准答案。
...
- 答案需要说明依据与结论之间的关系；
- 需要判断学生是否建立解释链、因果链或推理链；
- 不能只看结论是否相似，必须检查文本依据、解释过程和结论边界；
- Rubric 应明确 `requireTextEvidence`、`requireExplanation`、`requireConclusion` 等要求。
...
- 评价重点是表达任务是否满足指定约束；
- 需要检查内容完整性、语言规范、结构要求、风格一致性或表达效果；
- 不应按单一参考答案判断；
- 应通过 Rubric 描述表达目标、限制条件和可接受表现。
...
- 任务目标是训练或验证某个局部能力动作；
- 输出可以是文本片段、标注、修改稿、补充步骤或局部说明；
- 评价重点是学生是否完成指定学习动作，而不是完整开放题答案；
- 可用于 guided practice、revision、diagnostic verification 或 retest 前准备。
