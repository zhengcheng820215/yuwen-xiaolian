...
工程状态：PASS（16.1A Debug PASS；16.1B Human Demo PASS；UX 待后续专项优化）
...
工作台中的必填项必须显式标明。条件必填项必须说明触发条件；可保存未完成 Draft，但在执行校验、提交审核或 Freeze 前，不得依赖用户试错来发现字段门槛。
...
> Phase 16.1A 资源准入 Runtime 已通过 Debug 验收；Phase 16.1B 后续也已完成工程实现并通过人工 Demo 验收。Phase 16.1 的最小功能闭环现已标记为 PASS。
...
> Phase 16.1B 工程实现已经完成，并已由使用者按照“十三、16.1B 工作台人工验收”完成实际操作评审；最终验收结论见下一节。

## 二十一、16.1B 人工 Demo 验收记录（2026-07-19）

验收结果：**PASS（功能闭环通过，UX 存在较大欠缺）**。

通过依据：

- 无需修改工程代码即可创建并编辑 Structured Question Draft；
- 可以录入题目、Material、AnswerAcceptance、Rubric 与核心 Metadata；
- 必填项缺失和不合法组合能够被结构化校验阻断，并显示明确原因；
- 可以完成提交审核、通过、退回修改、拒绝和基于拒绝记录创建新修订 Draft；
- reviewed Draft 可以冻结为正式资源，并保留版本、审核和 Registry 追溯关系；
- 学生预览与审核预览能够呈现资源的不同使用视角；
- 已有 Debug `22 / 22 PASS`，Production Build 通过。

UX 评审结论：

- 当前工作台能够完成任务，但信息密度较高，操作路径和页面层级仍不够轻松；
- 表单需要进一步按步骤或内容分组，减少一次性认知负担；
- 条件必填字段、校验错误与对应输入项之间仍需要更直接的定位反馈；
- Draft、Review、Freeze、Revision 等状态需要更明确的下一步操作引导；
- 编辑、审核与预览区域的职责分工和切换体验仍需优化；
- 模板、默认值和重复录入辅助能力尚未形成，连续录题效率不足。

边界说明：

> 本次 PASS 只表示 Phase 16.1 所要求的资源准入 Runtime 与最小录入工作台功能闭环已经成立，不表示工作台 UX 已达到正式内容运营后台或高频生产工具标准。UX 欠缺不阻断 Phase 16.1 功能验收，但必须作为后续产品化优化项保留，不得因本次 PASS 被视为已解决。
