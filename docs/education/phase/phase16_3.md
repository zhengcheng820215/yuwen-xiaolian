...
- 当内部状态为 `review_required` 且没有真实人工承接流程时，不得显示“等待确认”。状态发生在正式保存前，学生端显示“本次结果暂不采用”；本轮正式结果已经保存、仅下一任务决策需要检查时，显示“本轮学习已经完成 / 下一任务待检查”，不得把本轮结果误报为未采用。
- 正向结果使用可追溯的“思路点评”，以本题正式任务要求的结构化覆盖检查区分“回答到位”与“还需补充”；内部保留 `covered / partially_covered / missing / insufficient_to_judge` 全部状态，学生端只显示最多一至两个到位点和一个主要缺口。不得以“正式记录”“受限反馈”、能力码、参考答案关键词匹配或泛化表扬替代学生可理解的内容。实现与页面验收记录见：[Phase 16.3 学生思路点评优化验收记录](./reports/phase16_3_student_thinking_feedback_debug_2026-07-21.md)。
