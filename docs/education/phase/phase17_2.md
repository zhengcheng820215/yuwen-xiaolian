...
12. 同一 Material Observation Plan 下，题号由 `observationTaskPlanId` 在计划中的顺序确定；待审核与已发布只是状态筛选，不得分别从“题目一”重新编号；
13. 待审核题目与已发布练习明细均按统一题号升序展示；题目从待审核流转为已发布时题号保持不变，列表允许因状态筛选出现不连续题号。
14. 当前素材选择是会话级导航状态：显式 URL 上下文优先，其次沿用当前页面选择，再次使用同一浏览器标签页的 `sessionStorage` 记忆；切换素材入口页卡不得清空最后选中的 active Material；
15. 会话恢复只记录 `materialVersionId` 与可选 `planId`，不得记录未保存素材表单、任务编辑缓冲或 AI Candidate；恢复时必须重新校验素材仍为 active、Plan 仍属于该素材，失效记录立即清除。
...
- 当前素材范围与跨状态统一题号专项 Debug `10 / 10 PASS`；
- 《皇帝的新装》实际状态投影：已发布为“题目一、题目三”，待审核为“题目二”，两侧均按统一题号升序且不因状态筛选重编号；
- 2026-07-30 已完成当前素材会话恢复：素材入口页卡切换不清除当前 active Material；离开工作台后在同一浏览器标签页返回时恢复最后素材与有效 Plan；审核返回或显式深链继续以 URL 上下文为最高优先级；
- 已停用、已删除或版本不存在的素材不会从会话恢复，失效 Plan 自动回落到该素材当前有效 Plan；会话中不保存素材正文、未保存表单、任务编辑缓冲或 AI Candidate；
- 素材选择恢复专项 Debug `9 / 9 PASS`，原工作台状态回归 `11 / 11 PASS`；
- 真实浏览器闭环通过：选择《皇帝的新装》后切换“素材录入 / 已有素材”仍保持选择，离开工作台再返回仍恢复《皇帝的新装》，显式《狼》深链能够覆盖会话记忆并打开《狼》；
...
## 2026-07-28 题目审核编辑与保存优化

题目审核编辑区进一步收紧为面向内容人员的操作语言：

1. Rubric 在界面统一表达为“评分标准”，内部观察项改为“评分项一 / 评分项二 / 评分项三”；
2. 每个评分项明确拆为“评分内容、对应能力、判定作用、答案要点”四类信息，并用“完整回答必须满足本项、需结合材料内容、需说明原因或过程”表达判断条件；
3. 质量提醒必须同时提供问题说明、具体修改位置、修改原则和非固定格式参考写法，避免只给工程代码或抽象结论；
4. 材料依据按“证据边界是否清楚”判断，允许局部范围、全文范围和开放取证三种合法方式，不强制套用段落号模板；
5. “保存本次修改”位于完整表单末尾，确保用户复核题干、评分标准、最低作答要求和来源后再提交保存；
6. 页面以正式业务字段生成内容签名。当前内容与最近一次成功保存一致时显示“当前内容已保存”并禁用按钮；发生有效变化后才恢复保存；
7. 保存期间按钮保持禁用，防止连续点击；保存成功后提示重新检查题目，并使依赖旧 Revision 的质量检查失效；
8. 防重复保存不等于 Freeze。草稿允许继续编辑，已发布版本继续保留，只有审核通过并发布的正式题目进入冻结状态。

验证结果：

- Question Quality Assessment Debug `17 / 17 PASS`；
- 相同内容进入页面时保存按钮禁用；
- 修改正式字段后保存按钮恢复，未保存的临时修改离开页面后不会写入；
- Production Build `PASS`，保留既有非阻断 Bundle Warning。

## 2026-07-28 题目问题逐项修复编排（P2）

当一道题同时存在多个质量提醒时，审核平台不再要求内容人员自行判断处理顺序，也不要求每修改一项就保存和检查一次。问题修复统一采用“逐项处理、集中复检”的队列：

1. 所有提醒按稳定顺序进入修复队列，界面明确标出当前优先处理项、待处理数量、待复检数量和总体进度；
2. 当前项完成修改后进入“等待重新检查”，下一项自动成为当前处理项；
3. 尚未处理的问题提供“开始修改”，已完成修改的问题不重复提供编辑入口；
4. 全部问题处理完成后，只执行一次“保存本次修改”和一次“保存并检查题目”，避免反复保存、重复生成 Revision 或打断修复思路；
5. 重新检查后，已解决项进入 resolved，仍存在的问题重新进入 pending；同一问题连续两次复检仍未解决时，继续使用 P1 的升级说明和人工处理建议；
6. 修复队列只编排操作顺序，不放宽质量规则，也不把“完成修改”误判为“检查通过”。

验证结果：

- Question Quality Revision Progress Debug `9 / 9 PASS`；
- Question Quality Assessment Debug `22 / 22 PASS`；
- Question Stem Optimization Debug `10 / 10 PASS`；
- Rubric Item Optimization Debug `8 / 8 PASS`；
- Production Build `PASS`，保留既有非阻断 Bundle Warning。

## 2026-07-30 检查层级文案与状态收敛

题目录入端明确区分“基础结构检查”和“完整质量检查”，并收敛原先容易形成矛盾感的多套状态文案：

1. 基础结构检查只表达字段和结构是否满足最低条件，不再独立代表整题质量通过；
2. 顶部状态优先合并展示“基础结构通过，N 项质量提醒待处理”；
3. 完整质量检查卡片直接展示通过项和待处理项数量；
4. 仅当当前 Revision 的完整质量检查无待处理项时，才显示“质量检查通过”；
5. 页面步骤、结果卡和相关提示统一使用“基础结构检查 / 完整质量检查”术语。

## 2026-07-28 题目问题逐项修复可靠性加固（P2.1）

P2.1 继续修复异常题目链路中“刷新后丢失、一次编辑误推进多项、修改位置不够精确”的问题：

1. 修复进度作为草稿正式字段持久化，不再依赖 `localStorage`。切换页面、刷新或重新进入草稿后，待处理、待复检和已解决状态均可恢复；
2. 每次只允许处理当前问题。后续问题保持等待状态，不能越过当前项直接操作；
3. 编辑字段与质量检查项建立显式映射。修改题干、难度或评分标准时，只使当前且确实受该字段影响的问题进入“等待重新检查”；
4. 当前问题一旦开始编辑即锁定。连续输入不会因为多次字段变更而误把后续问题逐项标记为已修改；
5. 评分标准类问题定位到具体评分项。重复评分项优先定位到发生重复的卡片，区分度问题定位到需要拆分的评分项；
6. 重新检查仍采用同一批次集中执行。检查消失的问题转为已解决，仍存在的问题保留并累计复检次数；
7. 异常链回归覆盖：无关字段编辑、宽字段编辑、连续输入、顺序推进、刷新恢复、部分解决与未解决重试。

验证结果：

- Question Quality Revision Progress Debug `18 / 18 PASS`；
- Question Quality Assessment Debug `22 / 22 PASS`；
- Question Resource Intake Debug `24 / 24 PASS`；
- Production Build `PASS`，保留既有非阻断 Bundle Warning。

## 2026-07-28 异常题目修订闭环与素材上下文收敛

本轮继续收紧“正常题目可以顺利发布，但异常题目容易反复建稿、无法定位修改位置或在发布末端才被拦截”的产品风险：

1. 质量提醒从抽象结论改为可执行修订任务，必须说明问题、修改位置、修改原则和可参考写法；题干、难度、能力映射与具体评分项使用显式字段映射；
2. 同一题目的修订采用定向 Revision，不重新生成整批训练任务。重复进入发布未完成环节时继续处理当前 Draft，不新增同题待审核草稿；
3. 题干与评分项分别提供受控 AI 优化。评分项优化必须结合题干、当前项及同题其他评分项，避免生成与相邻评分项重复的判断内容；
4. 保存内容与执行检查是两个明确动作：先保存当前 Revision，再基于该 Revision 重新执行质量检查；未保存修改不得沿用旧 Assessment；
5. 发布前先计算训练计划一致性和正式发布阻断项。存在能力、任务角色或难度差异时，在用户执行发布前明确展示差异及定向修复入口，不允许到发布结果页才首次告知；
6. 素材录入平台负责 Material Observation Plan 与训练任务规划；题目审核平台沿用计划值，只暴露当前题目确需修订的字段，避免两个平台重复维护同一组能力、角色和难度；
7. 当前素材摘要增加只读“素材内容”：默认显示前四个自然段，正文与操作文字统一为 14px，可通过唯一入口展开或收起全文；
8. 素材正文不在训练任务编辑区重复渲染。打开重新编辑后仍复用同一只读正文上下文，避免页面同时出现两套全文；
9. 切换素材时正文恢复默认收起状态；未选择素材时正文、题目统计和下游生产模块均不显示。
...
## 2026-07-28 审核定位与发布修订可靠性补强

本轮继续处理异常题目修订中“定位修改无响应、重复进入修订链生成新草稿、审核平台与素材平台计划值不一致”的问题：

1. Material Observation Plan 是主要能力、任务用途和难度的唯一计划来源。题目审核平台在计划审核模式下只读沿用这些值，保存草稿时再次按对应 Observation Task 归一化，避免旧表单值或重复字段覆盖训练计划；
2. 发布未完成题目的修订链使用稳定的修订根标识。重复进入同一题目的修复流程时优先复用当前有效修订稿，不再沿着上一份修订稿继续生成多层草稿；
3. 创建发布修订稿后立即重新执行发布前一致性检查。只有能力、任务用途和难度已经与训练计划一致时才进入后续审核；同步失败必须明确报错，不显示误导性的“已同步”状态；
4. “定位修改”只能指向当前页面模式中真实存在且可编辑的控件。材料依据、作答范围和计划难度提醒在计划审核模式下定位到题干；评分标准提醒定位到具体评分项；
5. 定位目标支持有序回退。具体评分项不存在时回退到评分标准模块，当前模式没有独立难度编辑器时回退到题干；所有目标均不存在时显示可见错误提示，不再静默无响应；
6. 定位成功后滚动到目标、聚焦可编辑控件并短暂高亮，用户可以确认系统实际指向的修改位置。
...
- Question Quality Revision Progress Debug `18 / 18 PASS`；
- Production Build `PASS`，保留既有非阻断 Bundle Warning；
- 浏览器真实交互验证：点击“定位修改”后页面滚动至题干编辑区，焦点进入题干输入框并显示高亮；
- `git diff --check` `PASS`。

## 2026-07-28 训练任务确认与题目审核交接优化

素材资源录入平台进一步收敛训练任务从生成、人工校准到题目审核的操作层级：

1. 训练任务卡片默认只读。AI 生成内容明确标记来源，人工主动进入编辑后才允许修改，避免浏览时误改正式候选内容；
2. 任务卡片首层采用固定顺序“能力目标 -> 题目 -> 学生任务 -> 观察目标”：能力目标说明训练什么，学生任务说明学生需要完成的具体动作，观察目标说明系统通过哪些可观察表现验证本题价值；四者不得使用近义文案相互重复；
3. 首层标签与内容保持 `8px` 间距，信息组之间保持约 `20px` 间距；评分标准与答案示例、设计依据、任务属性分别折叠，降低连续审核时的信息负担；
4. 每个任务显示 AI 生成、待审核、需修改或人工审核通过等状态；已通过任务默认折叠，存在问题的任务优先展开；
5. 批次顶部汇总任务数量、通过数量和问题清单，并提供稳定的“去修改”定位入口；
6. 原“确认训练任务”模块改为“提交题目审核”，不再重复展示前一模块已经核对过的完整题目列表；
7. 提交页只展示当前版本、任务数量、能力覆盖、训练方向、学习材料、材料范围和提交前检查；
8. 提交前检查必须明确训练任务是否具备能力目标、观察目标、评分标准、作答范围以及当前 Revision 的结构校验；
9. 未保存修改或检查未通过时阻断提交，并保留“返回任务调整”；通过后使用唯一主操作“提交题目审核”创建或恢复当前 Plan 对应的 Question Draft；
10. 工作台状态术语统一为“待审核资源”，避免将包含 Question、Rubric、Answer Acceptance 和质量 Assessment 的审核对象误解为只有题干；
11. “重新发布训练任务”改为“重新生成训练任务”，准确表达该操作只重建训练任务候选，不直接形成正式发布结果。

本轮只调整信息架构、文案、只读与编辑状态及既有动作的页面编排，不修改 Material Observation Plan、Question Draft、Review、Freeze、Registry 或 Learning Runtime 的正式职责。
...
## 2026-07-30 素材保存与训练任务规划状态解耦

本轮修复“只保存素材、尚未执行 AI 规划，却出现 3 个训练任务需要修改”的错误状态投影。

问题原因：

1. 无有效 Material Observation Plan 时，页面曾初始化 3 个空白训练任务占位项；
2. 训练任务检查把这些 UI 占位项误识别为真实业务任务；
3. 空白题干因此触发“请补充学生实际看到的题目”，让用户误以为保存素材后还需要修改并不存在的任务。

修复与冻结规则：

1. 保存 Material 后、AI 规划前，训练任务集合保持为空，页面显示“规划训练任务”；
2. 空任务集合不渲染训练任务检查、质量提醒或“去修改”入口；
3. `savePlanRevision` 在任务数为 `0` 时阻断，并提示先通过 AI 规划和候选采用建立真实任务；
4. 只有 AI 生成并采用任务，或从已有有效 Plan 恢复真实任务后，才启动覆盖统计和质量检查；
5. Material 保存、训练任务规划、任务检查与 Plan Revision 写入继续保持独立生命周期。

验证结果：

- Material Resource Production Command Debug `7 / 7 PASS`；
- 新增“仅保存素材不会把空白占位任务当成可保存任务组”回归；
- Production Build `PASS`，保留既有非阻断 Bundle Warning；
- 真实页面验收仍需确认：保存新素材后应显示 `0` 个任务且无质量提醒，AI 规划并采用后才出现任务检查。

## 2026-07-30 首次训练任务规划语义修复

本轮修复首次使用素材生成训练任务时，空任务组被错误展示为“当前任务组”，并出现“保留当前任务组 / 用候选组替换当前任务组”的问题。

问题原因：

1. 首次整组生成与已有任务组重新规划复用了同一个 `replace_group` 候选操作；
2. 数据层复用合理，但页面没有根据当前真实任务数区分首次规划与整组替代；
3. 当前任务数为 `0` 时仍渲染替代比较结构，制造了不存在的保留与替换对象。

修复与冻结规则：

1. 当前真实任务数为 `0` 时进入首次规划模式，标题显示“首批候选训练任务”；
2. 首次规划只提供“放弃候选 / 采用这组候选”，不展示空任务组比较，也不出现保留或替换文案；
3. 采用首批候选只写入编辑缓冲区，保存前不创建工作草稿或 Plan Revision；
4. 放弃首批候选后继续保持 `0` 个任务，并返回 AI 规划入口；
5. 只有当前真实任务数大于 `0` 时，才展示“替代候选任务组”“保留当前任务组”和“用候选组替换当前任务组”；
6. 空白 UI 占位项不参与模式判断。

验证结果：

- Training Task Group Planning Debug `PASS`；
- 新增空任务组采用首批候选的回归；
- `git diff --check` `PASS`；
- Production Build `PASS`，保留既有非阻断 Bundle Warning；
- 真实页面文案与按钮视觉验收仍为 `PENDING`。
...
1. 题目提交包含多个共享资源写入操作，短时间内可能发生 Shared Store 乐观并发竞争；
2. 客户端曾连续立即重试，竞争持续时容易耗尽重试次数；
3. Shared Store 的 Revision 竞争被旧错误映射误判为正式资源 Revision 冲突，并把报错文本中的 `expected` 错提取为对象 ID；
4. 保存按钮的动态深色背景类覆盖了通用 `disabled:` 视觉类，造成功能状态与视觉状态不一致。

修复与冻结规则：

1. `SHARED_RESOURCE_REVISION_CONFLICT` 属于共享存储短暂写入竞争，使用指数退避和轻量随机抖动自动重试，最多 6 次；
2. 自动重试只处理 Shared Store 写入竞争，不放宽 Draft、Assessment、Human Review、Freeze 或正式资源的业务 Revision 一致性检查；
3. 重试耗尽后统一返回 `SHARED_STORE_REVISION_CONFLICT`，提示“共享数据仍在同步，本次操作尚未完成，请稍后重试”；
4. 技术错误码折叠到“查看技术信息”，首层只显示用户可理解的结果与恢复动作；不得再显示虚假的“对象：expected”；
5. 无内容变化时，“当前内容已保存”必须同时具备原生 `disabled`、灰色背景与文字、浅灰边框和禁止操作指针；字段产生有效变化后才恢复主按钮样式；
6. 禁用样式由按钮可用状态直接决定，不依赖可能被动态业务样式覆盖的通用透明度或变体优先级。

验证结果：

- Structured Runtime Error Debug `9 / 9 PASS`；
- Shared Formal Resource Debug `12 / 12 PASS`，包含并发写入自动重试且无数据丢失；
- Material Question Review Submission Debug `5 / 5 PASS`；
- Production Build `PASS`，保留既有非阻断 Bundle Warning；
- 应用内浏览器真实重试提交成功，未再次出现冲突，并返回同一 `materialVersionId + planId` 素材上下文；
- 应用内浏览器确认“当前内容已保存”按钮为原生禁用且呈灰色禁用态。

## 2026-07-30 题目审核批次统计驻顶范围收敛

本轮修复题目审核页滚动时“批次处理概览”与四项核心统计一起驻顶、持续占用较大纵向空间的问题。

修复与冻结规则：

1. 驻顶区域只保留“本批题目、待人工审核、审核通过、已发布”四项核心生命周期统计；
2. “批次处理概览”继续展示阻断、提醒、待复检、重复修改与处理效率信息，但移出驻顶容器并随正文滚动；
3. 四项统计继续使用统一展示状态投影，不改变生命周期计数口径；
4. 驻顶容器保留不透明背景、底部分隔和稳定高度，不遮挡题目内容、问题处理与审核操作；
5. 该调整只收敛信息层级和滚动行为，不新增状态、Revision、Assessment 或审核流程。

验证结果：

- Production Build `PASS`，保留既有非阻断 Bundle Warning；
- 应用内浏览器确认核心统计容器为 `sticky`；
- 应用内浏览器确认“批次处理概览”存在且不位于任何 `sticky` 容器内；
- 页面滚动时仅四项核心统计保持可见，辅助概览随正文离开视口。

## 2026-07-30 审核发布完成态与决策区信息收敛

本轮继续收敛题目审核平台的完成状态表达和审核决策区信息层级。

修复与冻结规则：

1. 审核与发布步骤 2、3、4 固定外显，当前进行中的步骤使用蓝色；
2. 正式发布成功后不再保留蓝色当前步骤，第 4 步改为绿色“已完成发布”；
3. 已发布状态不再显示发布按钮，避免重复创建正式资源；
4. 提交人、提交时间、题目版本和状态只在顶部提交信息区展示；
5. 审核决策区删除重复的提交人和提交时间；
6. “审核说明（可选）”保留字段标题和占位文案，删除重复辅助句，默认不预填审核结论。

验证结果：

- Production Build `PASS`，保留既有非阻断 Bundle Warning；
- 应用内浏览器确认已发布题目显示绿色“已完成发布”，且不存在“发布正式题目”按钮；
- 应用内浏览器确认提交人信息只显示一次；
- 应用内浏览器确认审核说明输入框仍可用，重复辅助文案已删除。
