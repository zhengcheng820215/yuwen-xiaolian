...
题目审核与发布平台的页面规范与“素材资源录入平台”保持一致：使用 1360px 最大内容宽度、白色吸顶顶栏、浅灰页面背景、白色工作区、6px 圆角和绿色主操作；审核批次采用“题目导航 + 当前题目内容与评分标准 + 审核操作”的结构。该规范只作用于带 Plan 上下文进入的审核模式，通用题目录入工作台保持原有入口和功能。
