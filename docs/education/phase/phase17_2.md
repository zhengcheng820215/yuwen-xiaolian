...
2. 任务卡片首层只保留状态、属性、能力目标、题目和学生需要完成的动作；评分标准、答案示例、设计依据与任务属性分别折叠，降低连续审核时的信息负担；
3. 每个任务显示 AI 生成、待审核、需修改或人工审核通过等状态；已通过任务默认折叠，存在问题的任务优先展开；
4. 批次顶部汇总任务数量、通过数量和问题清单，并提供稳定的“去修改”定位入口；
5. 原“确认训练任务”模块改为“提交题目审核”，不再重复展示前一模块已经核对过的完整题目列表；
6. 提交页只展示当前版本、任务数量、能力覆盖、训练方向、学习材料、材料范围和提交前检查；
7. 提交前检查必须明确训练任务是否具备能力目标、观察目标、评分标准、作答范围以及当前 Revision 的结构校验；
8. 未保存修改或检查未通过时阻断提交，并保留“返回任务调整”；通过后使用唯一主操作“提交题目审核”创建或恢复当前 Plan 对应的 Question Draft；
9. 工作台状态术语统一为“待审核资源”，避免将包含 Question、Rubric、Answer Acceptance 和质量 Assessment 的审核对象误解为只有题干；
10. “重新发布训练任务”改为“重新生成训练任务”，准确表达该操作只重建训练任务候选，不直接形成正式发布结果。
