...
- [Phase 17.4 Local Shared Formal Resource Persistence](./phase/phase17_4.md) 已完成 A / B 拆分。`17.4A Shared Store Cutover` 的 Shared Store、Local API、Repository Adapter、显式基线初始化和基本备份已实现，专项 `8 / 8 PASS`，相关资源链回归 `88 / 88 PASS`，生产构建通过；当前为 `ENGINEERING + AUTOMATED DEBUG PASS / HUMAN BASELINE CUTOVER PENDING`。恢复规模化录题前仍需分别导出两侧真实快照、人工指定正式基线并完成双浏览器一致性验收。`17.4B Migration and Recovery Hardening` 为 `P2 / PLANNED`，后续补齐复杂迁移报告、冲突分析、历史快照、自动备份与恢复。
- [Phase 17.5 Question Generation Quality Assessment](./phase/phase17_5.md) 的 17.5A 已完成 Schema、七项确定性质量检查、Revision 失效规则、不可变 Repository 与专项 `12 / 12 PASS`；17.5B 已完成审核页质量摘要、Warning 展示、Human Review / Freeze 当前 Assessment 消费门与专项 `9 / 9 PASS`。Question Admission `22 / 22`、17.2 Generator `38 / 38` 及 Build 均通过。独立语义评估、批次摘要、Shared Store 正式持久化与固定 10 篇材料校准尚未完成。质量评估只提供可解释审核建议，不自动改题、删题、审核、冻结，也不替代 Phase 16.2 Resource Matching Quality Gate。
- 当前暂不移动目录。
