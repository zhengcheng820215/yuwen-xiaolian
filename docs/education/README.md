...
- [产品复杂度收口契约](../product/PRODUCT_COMPLEXITY_CONVERGENCE_CONTRACT.md)已完成阶段 0—4 工程与启动前预检，并于 2026-08-25 进入 `REAL TRIAL ACTIVE / OBSERVE ONLY`。该契约不是阶段 5 Training Model 开发，而是跨阶段收敛页面投射、条件触发、反馈表达和能力退役复杂度；主链、Frozen Resource、Evidence 隔离、Session Snapshot、幂等、Outbox 与 Registry Version Chain 保持冻结。当前只是单学生、21 天封闭观察开始，不代表复杂能力价值或教育效果已经得到验证。
...
- [复杂度收口阶段 3 反馈与 Profile 投射收口工程实施与 Debug 验收文档](../product/PRODUCT_COMPLEXITY_CONVERGENCE_STAGE3_FEEDBACK_PROFILE_PROJECTION_ENGINEERING_AND_DEBUG_PLAN.md)与[工程、Debug 和浏览器验收报告](./phase/reports/product_complexity_convergence_stage3_engineering_debug_browser_acceptance_2026-08-24.md)已经完成，状态为 `ENGINEERING COMPLETE / DEBUG ACCEPTED / FULL BROWSER ACCEPTED`。阶段 3 只从既有正式 Feedback、Diagnosis、Requirement Coverage、Revision Evaluation 与 `StudentAbilityProfile` 形成学生反馈和 `CoreAbilitySummary` 只读投射；反馈最多呈现一个真实完成点、一个正式主要缺口和一个可执行动作，但不机械要求固定三段。`C3-01—C3-48` 为 `48 / 48 PASS`，`B3-01—B3-18` 为 `18 / 18 PASS`，旧主链专项回归 `405 / 405 PASS`，Production Build PASS，正式资源、Attempt、Evidence、Profile 与真实校准分母写入均为 0。真实反馈收益和能力概况价值仍须由阶段 4 封闭试用验证。
- [复杂度收口阶段 4 稳定试用与退役决策工程实施与 Debug 验收文档](../product/PRODUCT_COMPLEXITY_CONVERGENCE_STAGE4_STABLE_TRIAL_AND_RETIREMENT_ENGINEERING_AND_DEBUG_PLAN.md)与[工程、Debug 和浏览器验收报告](./phase/reports/product_complexity_convergence_stage4_engineering_debug_browser_acceptance_2026-08-25.md)已经完成，状态为 `ENGINEERING COMPLETE / DEBUG ACCEPTED / FULL BROWSER ACCEPTED / ISOLATED OBSERVATION READY / ACTIVATION PREFLIGHT REQUIRED / REAL TRIAL NOT STARTED`。阶段 4 只从既有正式事实形成来源隔离的 Append-only Observation、版本化 Aggregate 和内部 Decision Proposal，不增加 Training Model 或调度状态机；`C4-01—C4-64` 为 `64 / 64 PASS`，`B4-01—B4-20` 为 `20 / 20 PASS`，旧主链专项回归 `295 / 295 PASS`，Production Build PASS。浏览器验收中正式资源、Attempt、Evidence、Profile 与真实试用分母写入均为 `0`。该工程证明隔离观察基线就绪；启动 14—28 日真实 Trial Window 前仍须完成生产接线和 `RTW-S01—RTW-S18` 预检，Fixture、Debug 与浏览器验收数据不得替代真实证据。
- [复杂度收口阶段 4 真实 Trial Window 启动与运行验收契约](../product/PRODUCT_COMPLEXITY_CONVERGENCE_STAGE4_REAL_TRIAL_WINDOW_OPERATION_AND_ACCEPTANCE_CONTRACT.md)已完成，状态为 `DESIGN FROZEN / REAL TRIAL ACTIVE / OBSERVE ONLY`。文档冻结了 `RTW-S01—RTW-S18` 启动门槛、真实数据准入、活跃日和中期巡检、P0—P2 问题分级、窗口关闭/失效顺序、`RTW-E01—RTW-E16` 结束验收和单学生解释边界。启动门槛已于 2026-08-25 签署并通过；真实试用完成仍不等于教育效果已经证明。
- [复杂度收口阶段 4 真实 Trial Window 启动前工程实施与 Debug 验收文档](../product/PRODUCT_COMPLEXITY_CONVERGENCE_STAGE4_REAL_TRIAL_PREFLIGHT_ENGINEERING_AND_DEBUG_PLAN.md)与[工程、Debug 和浏览器验收报告](phase/reports/product_complexity_convergence_stage4_real_trial_preflight_engineering_debug_browser_acceptance_2026-08-25.md)已经完成；`PF-C01—PF-C56` 为 `56 / 56 PASS`，`PF-B01—PF-B20` 为 `20 / 20 PASS`，阶段 0—4、Learning 主链与 Production Build 通过。[真实 Trial 激活签署与执行记录](phase/reports/product_complexity_convergence_real_trial_activation_signoff_2026-08-25.md)进一步确认单学生 21 天 Window、不可变 Preflight Report、`approved_to_activate` Launch Record 与浏览器激活均已完成；当前 requested / effective mode 为 `real_trial / real_trial`，八项能力均为 `observe_only`，初始 Observation Event 为 0。
- 当前暂不移动目录。
