...
import {
  ...
  optimizeRubricItem,
} from '../ai/agents/rubricItemOptimizationAgent.ts';
...
export function createRubricItemOptimizationBoundary(): Connect.NextHandleFunction {
  ...
  return async (request, response) => {
    ...
    try {
      ...
      const input = body.input as RubricItemOptimizationInput;
      const model = process.env.DEEPSEEK_MODEL?.trim() || 'deepseek-v4-flash';
      const provider = new DeepSeekChatDiagnosisProvider({ apiKey: credential.apiKey });
      const result = await optimizeRubricItem(input, {
        provider,
      ...
      response.end(JSON.stringify({
        ...
        code: 'rubric_item_optimization_failed',
        retryable: false,
      }));
    }
  };
