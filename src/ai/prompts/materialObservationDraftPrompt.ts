...
export const MATERIAL_OBSERVATION_DRAFT_PROMPT_VERSION = 'material_observation_draft_prompt_v1_2' as const;

export function buildMaterialObservationDraftPrompt(
  input: MaterialObservationDraftGeneratorInput,
): string {
  const candidateCount = clamp(input.preferences?.candidateCount ?? 4, 3, 6);
...
当前生成模式固定为 discover_new_observation。请先检查已有 Observation 与 Question Inventory，再基于材料生成最多 ${candidateCount} 个尚未覆盖、彼此具有独立观测价值的 Training Candidate。找不到足够的新 Observation 时宁可少生成，不得用改写题干凑数量。
...
8. Rubric 必须描述可观察的作答步骤，不能只写“回答正确”。
...
<material id="${escapeAttribute(input.material.materialVersionId)}" title="${escapeAttribute(input.material.title)}">
