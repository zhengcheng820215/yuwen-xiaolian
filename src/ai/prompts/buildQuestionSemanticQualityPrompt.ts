...
export function buildQuestionSemanticQualityPrompt(input: {
  ...
  const payload = {
    ...
    question: {
      ...
      choiceInteraction: input.draft.choiceInteraction,
  ...
  return [
    ...
    'evidenceRefs 只能引用：material.title、material.content、draft.questionStem、draft.questionType、draft.responseFormat、draft.options、draft.choiceInteraction、draft.answerAcceptance、draft.rubric、draft.minimumAnswerRequirement、draft.abilityMetadata。',
    '可以使用“字段:简短摘录”、合法子字段路径（如 draft.choiceInteraction.correctOptionIds）或数组路径（如 draft.options[0]）形式，不得编造字段。',
    '若 responseFormat 为 single_choice，还须核对正确项、各错误项的可解释偏差、答案接受范围与题干观察目标是否一致；不能仅依据题干完成判断。',
