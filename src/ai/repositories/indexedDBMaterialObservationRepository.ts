...
export class IndexedDBMaterialObservationRepository implements MaterialObservationRepository {
  ...
  async savePlan(value: MaterialObservationPlan) {
    ...
    if (existing?.status === 'reviewed' && JSON.stringify(existing) !== JSON.stringify(value)) throw new Error('Reviewed MaterialObservationPlan is immutable. Create a new revision.');
