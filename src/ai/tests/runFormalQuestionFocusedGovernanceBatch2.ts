...
import {
  ...
  prepareFormalQuestionFocusedGovernanceBatch2,
...
const prepared = prepareFormalQuestionFocusedGovernanceBatch2(
  before.data,
...
const governedMaterialVersionIds = new Set(successorVersions.map((version) => version.materialVersionId));
...
if (!prepared.report.alreadyApplied) {
  assert(audit.dispositionBreakdown.decompose_or_refocus
    <= beforeAudit.dispositionBreakdown.decompose_or_refocus - 3);
  assert(audit.findingBreakdown.composite_core_actions
    <= beforeAudit.findingBreakdown.composite_core_actions - 3);
}
...
const marked = prepared.data.questionResources.versions.filter((version) => (
  ...
  && version.tags.includes(FORMAL_QUESTION_FOCUSED_GOVERNANCE_BATCH2_MARKER)
...
if (apply && !prepared.report.alreadyApplied) {
  const committed = await store.applyCommand(before.revision, {
    ...
    commandId: 'formal-question-focused-governance-2026-08-21-batch-2-v1',
    patches: [
...
console.log('Formal question focused governance Batch 2 passed.');
