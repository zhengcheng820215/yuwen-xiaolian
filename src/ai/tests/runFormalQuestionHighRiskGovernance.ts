...
import {
  ...
  prepareFormalQuestionHighRiskGovernance,
...
const prepared = prepareFormalQuestionHighRiskGovernance(
  before.data,
...
assert.equal(prepared.report.successorResourceVersionIds.length, 3);
assert.equal(governedResults.length, 3);
assert(governedResults.every((result) => result.disposition !== 'regenerate'));
assert(governedResults.every((result) => result.findings.every((finding) => (
  finding.code !== 'evidence_scope_insufficient'
  && finding.code !== 'response_format_load_mismatch'
  && finding.code !== 'relation_load_overloaded'
))));
assert(governedResults.every((result) => result.profile?.requiredEvidenceUnitCount === 1));
assert(governedResults.every((result) => (result.profile?.requiredRelationCount || 0) <= 1));
assert.equal(audit.findingBreakdown.evidence_scope_insufficient, 0);
assert.equal(audit.sequenceFindingBreakdown.unexplained_load_jump, 0);

const marked = prepared.data.questionResources.versions.filter((version) => (
  version.status === 'frozen'
  && version.tags.includes(FORMAL_QUESTION_HIGH_RISK_GOVERNANCE_MARKER)
...
if (apply && !prepared.report.alreadyApplied) {
  const committed = await store.applyCommand(before.revision, {
    ...
    commandId: 'formal-question-high-risk-governance-2026-08-21-batch-1-v1',
    patches: [
...
console.log('Formal question high-risk governance passed.');
