      ...
      <div className="px-5">
        ...
        <div className="grid gap-3">
          {categories.map((category) => (
            <Link key={category} to={`/quiz/${encodeURIComponent(category)}`} className="flex min-h-16 items-center justify-between rounded-lg bg-white px-4 shadow-sm">
              <div>
                <p className="font-medium text-slate-900">{category}</p>
                <p className="mt-1 text-xs text-slate-500">单选 / 判断 / 填空</p>
              </div>
